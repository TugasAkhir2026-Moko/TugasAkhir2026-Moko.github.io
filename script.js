...
// Menyimpan nilai totalDeteksi SEBELUM update terbaru, dipakai untuk
// mendeteksi kenaikan angka secara instan (agar grafik bisa "kedip naik"
// duluan tanpa menunggu data node 'logs' resmi dari Firebase).
let previousTotalDeteksi = 0;
...
function switchTab(tabId) {
    ...
    const titles = {
        ...
        sensors: 'Status Sensor Real-time',
...
function loadSavedCredentials() {
    ...
    if (savedFb && fbConfig.apiKey && fbConfig.databaseURL && fbConfig.projectId) {
        updateConnectionStatusUI('connecting');
        initFirebase();
    } else {
        updateConnectionStatusUI('unconfigured');
        showToast("Harap set up Firebase di tab Integrasi agar dashboard bisa live!", "error");
    }
}
    ...
    if (fbConfig.apiKey && fbConfig.databaseURL && fbConfig.projectId) {
        try {
            ...
            statusRef.on('value', snap => {
                ...
                applyRealtimeData({ ...latestStatus, logs: latestLogs });
            }, err => {
...
function applyRealtimeData(data) {
    ...
    $('sensor-pir-live').innerText = data.pir_sensor ? "Terdeteksi" : "Siaga";
    $('sensor-pir-live').className = `text-xs font-bold px-3 py-1.5 rounded-lg ${data.pir_sensor ? 'bg-red-100 dark:bg-red-950/40 text-red-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`;
    $('sensor-speaker-live').innerText = data.play_sound ? "Membunyikan" : "Siaga";
    $('sensor-speaker-live').className = `text-xs font-bold px-3 py-1.5 rounded-lg ${data.play_sound ? 'bg-orange-100 dark:bg-orange-950/40 text-orange-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`;
...
function triggerSoundRepeller() {
    ...
    if (!dbRef) {
        showToast("Firebase belum terhubung! Cek tab Integrasi.", "error");
        return;
    }
...
function saveFirebaseSettings() {
    ...
    showToast("Kredensial Firebase Disimpan! Menghubungkan...", "success");
